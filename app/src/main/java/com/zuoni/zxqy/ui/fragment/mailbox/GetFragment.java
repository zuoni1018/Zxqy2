...
public class GetFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        rvMailboxAdapter.setOnMailBoxListener(new OnMailBoxListener() {
            ...
            public void onClick02(Message message, int position) {
                ...
                mIntent.putExtra("message1", message.getName());
                mIntent.putExtra("message2", CacheUtils.getUserid(getContext()));
    ...
    private void get_message_tome() {
        String url = "/User/get_message_tome/p/1/size/10";
        if (!isFirst) {
            nowPage++;
            url = "/User/get_message_tome/p/" + nowPage + "/size/10";
        }
        myMailboxActivity.showLoading();
        HttpRequest httpRequest = new HttpRequest(AppUrl.BASE_URL + url);//收到的消息
