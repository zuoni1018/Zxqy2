...
public class XEditText extends AppCompatEditText {
    ...
    private Bitmap getBitmapFromVectorDrawable(Context context, int drawableId, boolean tint) {
        ...
        if (tint) DrawableCompat.setTint(drawable, getCurrentHintTextColor());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (hasFocused && mBitmap != null && isPwdInputType && !isTextEmpty()) {
            int left = getMeasuredWidth() - getPaddingRight() - mTogglePwdDrawable.getIntrinsicWidth() - mBitmap.getWidth() - dp2px(4);
